...
for (file in borealis_in) {
    ...
    assign(name, GenomicRanges::makeGRangesFromDataFrame(
        read.table(file.path(extdata,file), header=TRUE,
...
head(allCollapsed)
subset(allCollapsed, select=c(SampleID, annot.symbol, NoSites, fractionSignif),
    SampleID=="vignette_borealis_patient_72" & annot.symbol=="LTB4R" & 
    grepl("promoter", annot.id))
