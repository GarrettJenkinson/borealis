...
#signif_counts <- signif_only %>% dplyr::count(sampleID)
signif_counts <- dplyr::count(signif_only,sampleID)
...
annotFunc <- function(file,extracols,annots){
...
myAnnots <- c('_genes_promoters','_genes_1to5kb','_genes_cds','_genes_5UTRs',
...
allAnnot <- annotFunc(file.path(extdata,padjustedIn),extracols,myAnnots)
```
...
plotCpGsite("chr14:24780288",
    ...
    methCountFile=file.path(extdata,
                            "vignette_CpG_model_rawMethCount_chr14.tsv"),
    totalCountFile=file.path(extdata,
...
collapseFunc <- function(input,padjThresh=0.05) {
    ...
    return(featureCountsEnhancement(featurecnts,genemap,pvals))
}

featureCountsEnhancement <- function(featurecnts,genemap,pvals){
    newPvals <- lapply(seq_along(featurecnts$annot.id), 
                    function(i) {perFeatFunc(featurecnts$annot.id[i],pvals)})
...
allCollapsed <- collapseFunc(allAnnot)
```
