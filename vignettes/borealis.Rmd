...
# Run borealis
results <- runBorealis(extdata,nThreads=1, chrs="chr14", outprefix=paste(extdata,"/borealis_", sep=""), modelOutPrefix =paste(extdata,"/CpG_model", sep=""))
...
for (file in borealis_in) {
    ...
    assign(name, read.table(paste(extdata,file, sep="/"), header=TRUE, stringsAsFactors=FALSE))
}
...
for (i in 1:length(list_DataFrame)) {
    ...
    filepath <- paste(extdata,"/",list_object_names[i],'.padj',sep="")
...
mu_theta<-unique(subset(combined_files, select=-c(x,n,pVal, isHypo, pAdj, effSize, sampleID)))
```
...
```{r}
# Create table of unique positions and depth/p-val/padj for each position in each case
...
sig_quantiles<-quantile(signif_counts$n, probs=c(0.025, 0.05, 0.95, 0.975, 0.999))
...
annot_func<-function(file){
    ...
    # Select annnotation classes we want from annotatr (these can be user-customized
    ...
    write.table(finalout.df, file=filepath, quote=FALSE,sep='\t',row.names=FALSE)
}
```
...
plan(multisession)
future_lapply(paste(extdata,"/",c(padjusted_in),sep=""), function(i) {annot_func(i)}, future.seed=TRUE)
```
...
all_annot<-bind_rows(lapply(paste(extdata,"/",annots, sep=""), fread))
```
...
```{r sitegraph, fig.wide=TRUE, fig.cap="Single-site data summary provided by the plotCpGsite fucntion included with borealis", echo=FALSE}
# Use Borealis plotting function to investigate this site further
plotCpGsite("chr14:24780288",
sampleOfInterest="patient_72",
modelFile=paste(extdata,"/","/CpG_model_chr14.csv",sep=""),
methCountFile=paste(extdata,"/","/CpG_model_rawMethCount.tsv",sep=""),
totalCountFile=paste(extdata,"/","CpG_model_rawTotalCount.tsv",sep=""))
```
...
# Process each file, summarize data across features and output new files
    ...
    per_feat_func<-function(annot_id){
        ...
        pout<-pchisq(-2*sum(log(c(ss$pAdj))),df,lower.tail = F)
        pouthypo<-pchisq(-2*sum(log(c(sshypo$pAdj))),dfhypo,lower.tail = F)
        poutnohypo<-pchisq(-2*sum(log(c(ssnohypo$pAdj))),dfnohypo,lower.tail = F)
        medeshypo<-median(c(sshypo$effSize))
        medesnohypo<-median(c(ssnohypo$effSize))
        ishypo<-length(ss$isHypo[which(ss$isHypo=="TRUE" & ss$pAdj<=padj_thresh)])
        ishypo_all<-length(ss$isHypo[which(ss$isHypo=="TRUE")])
        ishyper<-length(ss$isHypo[which(ss$isHypo=="FALSE" & ss$pAdj<=padj_thresh)])
...
future_lapply(paste(extdata,"/",c(annots),sep=""), function(i) {collapse_func(i)})
```
...
collapsed<-paste(extdata,"/",c(collapsed),sep="")
