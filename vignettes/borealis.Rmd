...
combined_files <- listGRanges[[1]]
for (i in 2:length(listGRanges)) {
    combined_files <- c(combined_files, listGRanges[[i]])
}
```
...
featurecnts <- merge(locations, featurecnts, by.x="annot.id", 
    by.y="annot.id")
genemap <- cbind(allAnnot$annot.symbol, allAnnot$annot.id, allAnnot$annot.tx_id,
    allAnnot$annot.width, allAnnot$sampleID)
