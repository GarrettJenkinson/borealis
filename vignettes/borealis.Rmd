...
allAnnot <- dplyr::bind_rows(lapply(file.path(extdata,annots), data.table::fread))
```
...
# Process each file, summarize data across features and output new files
    ...
    newPvals <- future.apply::future_lapply(seq_along(featurecnts$annot.id), function(i) {
    perFeatFunc(featurecnts$annot.id[i])})
    ...
    newPvals <- tidyr::separate(newPvals, newPvals, into=c("isHypoSig", "isHypoAll",
