...
# Run borealis
results <- runBorealis(extdata,nThreads=2, chrs="chr14",
        suffix=".gz",
        outprefix=file.path(extdata,"vignette_borealis_"), 
        modelOutPrefix=file.path(extdata,"vignette_CpG_model"))
...
```{r, message=FALSE, warning=FALSE}
suppressMessages(require(dplyr)) # GARRETT: remove this?
...
for (i in seq_along(list_DataFrame)) {
    ...
    row.names=FALSE) ## GARRETT: REMOVE?
}
```
...
Now let's load the package and read in the file we created in the previous steps
...
padjustedIn <- dir(extdata,pattern="*.padj")

# Now we subset to the single patient for the vignette
padjustedIn <- padjustedIn[grep("patient_72",padjustedIn)]
```

Now we provide a function that will annotate each file with annotations from
`r Biocpkg("annotatr")`.

```{r}
# Function to annotate Borealis output with the annotatr package
annotFunc <- function(file){
    basename <- file
    ...
    annotations = annots)
    dmrs.annotatr.gr <- annotatr::annotate_regions(regions=dmrs.gr,
...
#future.apply::future_lapply(file.path(extdata,c(padjustedIn)), 
#        function(i) {annotFunc(i)}, future.seed=TRUE)
annotFunc(file.path(extdata,padjustedIn))
```

With the annotation complete, we can read in the new files for inspection.

```{r}
# Read in annotated file to summarize CpG data across features
annots <- dir(extdata,pattern="*.annot$")
allAnnot <- data.table::fread(file.path(extdata,annots)) # GARRETT: remove DT?
#allAnnot <- dplyr::bind_rows(lapply(file.path(extdata,annots), 
#                                    data.table::fread))
```
...
subset(allAnnot, pVal == min(allAnnot$pVal))
```
...
```{r sg, fig.wide=TRUE, fig.cap="plotCpGsite function", echo=FALSE}
# Use Borealis plotting function to investigate this site further
plotCpGsite("chr14:24780288",
sampleOfInterest="patient_72",
modelFile=file.path(extdata,"vignette_CpG_model_chr14.csv"),
methCountFile=file.path(extdata,"vignette_CpG_model_rawMethCount_chr14.tsv"),
totalCountFile=file.path(extdata,"vignette_CpG_model_rawTotalCount_chr14.tsv"))
```
...
First lets read in our annotated file and set a p-value we will use to determine
significance in our feature-summarized data. In our summarization we will make
...
Now we will create a function to do the summarization. You can use this as-is or
can easily customize it to create summary metrics specific to your application.

```{r}

# Function to calculate per feature metrics in parallel
perFeatFunc <- function(annot_id,pvals){
...
# Process each file, summarize data across features and output new files
    ...
    signifonly <- subset(input, pAdj<=padjThresh)
    ...
    featurecnts <- merge(locations, featurecnts, by.x="annot.id", 
                            by.y="annot.id")
    ...
    newPvals <- future.apply::future_lapply(seq_along(featurecnts$annot.id), 
                                                function(i) {
    perFeatFunc(featurecnts$annot.id[i],pvals)})
    ...
    newPvals <- tidyr::separate(newPvals, newPvals, into=c("isHypoSig", 
    "isHypoAll","isHyperSig", "isHyperAll", "isNA", "pAdj", "pAdjHypo", 
    "MedESHypo","pAdjHyper", "MedESHyper", "Best_ss_pAdj", "Best_ss_ES"), 
    sep=",")
    ...
    output1 <- paste(file, "_", padjThresh, "_feature_collapse.txt", sep="")
...
#future.apply::future_lapply(file.path(extdata,c(annots)),
#        function(i) {collapseFunc(i)})
collapseFunc(file.path(extdata,annots))
```
...
collapsed <- file.path(extdata,collapsed)
#allCollapsed <- dplyr::bind_rows(lapply(collapsed, data.table::fread))
allCollapsed <- data.table::fread(collapsed) #GARRETT: remove DT?

# Select the feature with the lowest individual p-value
subset(allCollapsed, Best_ss_pAdj == min(allAnnot$pAdj))
```
...
the CpG sites we have data for are called significant by Borealis! That's 35 CpG
